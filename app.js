...
// ======================================================
// 🎨 EFECTO DEGRADADO HSL CON MOUSE MOVE (ROJO/AMARILLO)
// ======================================================
...
document.addEventListener('mousemove', (e) => {
  ...
  // Si Y es 0 (arriba), el color es más hacia el rojo. Si Y es viewportHeight (abajo), el color es más hacia el amarillo.
  const normalizedY = e.clientY / viewportHeight;
  
  // Matiz (H): Va de rojo (0) a amarillo (60)
  // Cuando normalizedY es 0 (arriba), hue es 0 (rojo).
  // Cuando normalizedY es 1 (abajo), hue es 60 (amarillo).
  const hue = normalizedY * 60; // Rango de 0 (rojo) a 60 (amarillo)
  
  // Saturación (S): Mantenemos una saturación alta para colores vivos
  const saturation = 90; // Rango de 90% (bastante saturado)
  
  // Luminosidad (L): Mantenemos una luminosidad media-alta
  const luminosity = 50 + (1 - normalizedY) * 10; // Rango de 50% a 60% para que no sea muy oscuro
